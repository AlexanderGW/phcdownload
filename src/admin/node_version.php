...
switch( $kernel->vars['action'] )
{
	...
	case "version" :
	{
		...
		{
			$version_data = "<i>" . $kernel->ld['phrase_unknown'] . "</i>";
		}
		else
		{
	...
	default :
	{
		...
		else
		{
			...
			{
				$version_data = "<font color=\"#33cc33\">" . $kernel->config['short_version'] . "</font> (" . $kernel->ld['phrase_status_up_to_date'] . ")";
			}
			else
			{
				$version_data = "<font color=\"red\" title=\"Build: " . FULL_VERSION . "\">" . $kernel->config['short_version'] . "</font> (" . $kernel->ld['phrase_status_out_dated'] . ")";
			}
		}
